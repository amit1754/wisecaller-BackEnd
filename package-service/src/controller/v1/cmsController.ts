...
class CmsController {
  ...
  async show(req: Request, res: Response) {
    try {
      ...
      return res
        .status(200)
        .json({ success: true, message: "page  get successfully", data: page });
    } catch (error: any) {
      return res
        .status(500)
        .json({ success: false, messagcmse: error.message });
    }
  }
  ...
  async update(req: Request, res: Response) {
    try {
      ...
      let user = await CmsModel.findOneAndUpdate({ _id: id }, payload, {
        upsert: true,
        new: true,
      });
      return res
