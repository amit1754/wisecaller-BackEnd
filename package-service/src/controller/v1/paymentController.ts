  ...
  async add(req: Request, res: Response) {
    try {
      ...
      if (paymentSave) {
        res
          .status(200)
          .json({ success: true, message: "payment detilas updated" });
      } else {
      ...
      if (error.code === 11000) {
        res
          .status(400)
          .json({ success: false, message: "Payment details already exists" });
      } else {
  ...
  async show(req: Request, res: Response) {
    try {
      ...
      if (logginUser?.role != "ADMIN") {
        ...
        const data = await PaymentModel.find(whereClause)
          .populate(["user", "packageId"])

          .skip(page > 0 ? +limit * (+page - 1) : 0)
          .limit(+limit || 20);
        const totalCount = await PaymentModel.find(
          whereClause
        ).countDocuments();

        res.status(200).json({
          success: true,
          message: "paymentDetails get successfully",
          data,
          totalCount,
        });
      }
