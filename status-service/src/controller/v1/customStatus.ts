      ...
      for (let i = 0; i < length; i++) {
        if (body[i].is_deleted) {
          ...
          if(createTrigger){
            ...
            await CloudWatchRuleClient.createCloudWatchEvent(createRule,exp,eventPayload,process.env.STATUS_PROCESSOR_TOPIC_ARN)
          }
      ...
      if (body.status && body.status.length) {
        ...
        for (let i = 0; i < length; i++) {
          if (body.status[i].is_deleted) {
            ...
            var ruleName  =dateName+"_" +(loggedInUser._id?"_"+loggedInUser._id:"")+"_"+(body.status[i].customId?body.status[i].customId:(custStatus.status+"_"+custStatus.substatus));              
            await CloudWatchRuleClient.deleteRule(ruleName)            
            await customStatus.findOneAndRemove({
            ...
            var createTrigger =  (
                                (
                                  moment(body.status[i].start_date) < moment(utcDate).add(24,"hours") &&
  ...
  async expandAndSetRuleForCustomStatus(req: Request, res: Response) {
    try {
      ...
      if (diff <=15  || (refStatusId != null && createdUserId != null))
      {
        ...
        let where = {
            $and: [
              {start_date: { $lte: moment(utcDate).add(24,"hours").toISOString() }},
            ...
            for (const nonrecurring of getStatus) { 
              ...
              var day = utcDate.format('DD');
              var year = utcDate.format('YYYY');
              var ruleName  =dateName+"_" +(userId?"_"+userId:"")+"_"+(customName?customName:(statusId+"_"+subStatusId));
              var exp = 'cron('+min+' '+hour+' '+day+' '+month+' ? '+year+')';  
              var payload = {"rule":ruleName,
                "status_type":"CUSTOM",
            ...
            for (const recurringstatus of getStatusResurring) {    
              ...
              var diffDate = moment(recurringstatus.end_date).diff(moment(recurringstatus.start_date),"days");
              var endTime = moment(utcDate.add(diffDate,"days").format("YYYY-MM-DD") +" "+ rRule.endTime).toISOString();   
              ...
              if (freq == "DAILY"){
                ...
                if (dateValue != null && dateValue.length >0){
                  ...
                  if (dayValue != null && dayValue.length >0){
                    ...
                    if (dayValue.indexOf(dayIndex)>=0 && excatWeek.indexOf(weekIndex)>=0){
                      createRule = true;
                    }
                  } 
                  ...
                  if (dayValue != null && dayValue.length >0){
                    ...
                    if (dayValue.indexOf(dayIndex)>=0){           
                      ...
                      if (dateIteration <= utcDate){
                        var diff = moment(utcDate).add(24,"hours").diff(dateIteration, "days");
                ...
                var dayIndex = utcDate.day()+1;
                if (weekday!= null && weekday.length >0 && weekday.indexOf(dayIndex)>=0){
                  createRule = true;
                }
              } else if (freq == "YEARLY"){
                ...
                var day = utcDate.format('DD');
                if (dateValue!= null && dateValue.length >0 && (excatMonth == null || excatMonth.length <=0) && (onEveryMonth ==null || onEveryMonth.length <=0)){
                  var utcDayIndex = parseInt(day);
                  if (dateValue.indexOf(utcDayIndex)>=0){
                    createRule = true;
                  }
                  ...
                  {           
                    if (dateValue!= null && dateValue.length >0){
                      ...
                      if (dayValue!= null && dayValue.length >0 && yearDate.excatWeek!= null && yearDate.excatWeek.length >0){
                          var excatWeek = yearDate.excatWeek;  
                          if (dayValue.indexOf(dayIndex)>=0 && excatMonth.indexOf(monthIndex)>=0 && excatWeek.indexOf(weekIndex)>=0){
                            createRule = true;
                          }                                       
                      }
                    }                    
                  ...
                  if (dateValue!= null && dateValue.length >0){
                    ...
                    if (dateValue.indexOf(utcDayIndex)>=0) {
                      ...
                      if (dateIteration < utcDate){
                        var utcMont = moment(utcDate).add(24,"hours").month() + 1;
                        var eventStartDateMont= dateIteration.month() + 1;
                        var noOfmont  =  utcMont >eventStartDateMont ? utcMont - eventStartDateMont :eventStartDateMont-utcMont;
                      ...
                      if (dayValue!= null && dayValue.length >0 && yearDate.excatWeek != null && yearDate.excatWeek.length >0){
                          ...
                          if (dayValue.indexOf(dayIndex)>=0 && excatWeek.indexOf(weekIndex)>=0){
                            ...
                            for(var i =1;i<=days;i++){
                              ...
                              if (dayValue.indexOf(dayIndex)>=0 && excatWeek.indexOf(wkIndex)>=0){
                                break;
                              }
                              dateIteration.add(1,"days");
                            }       
              ...
              if (createRule){                
                ...
                var ruleName  =dateName+"_" +(userId?"_"+userId:"")+"_"+(customName?customName:(statusId+"_"+subStatusId));
                var exp = 'cron('+min+' '+hour+' '+day+' '+month+' ? '+year+')';   
                var payloadRecurring = {"rule":ruleName,
                  "status_type":"CUSTOM",
            ...
            if (refStatusId == null ){
              ...
              const worklife: any = await WorkLifeBalance.find(
                whereWorkBalance );
                for (const wrkLife of worklife) { 
                  ...
                  var endDateTime = moment(utcDate.format("YYYY-MM-DD") +" "+ wrkLife.end_time).toISOString();   
                  var hour = schedulingTime.format('HH') ;
                  var min = schedulingTime.format('mm');
                  var month = utcDate.format('MM');
                  var day = utcDate.format('DD');
                  var year = utcDate.format('YYYY');
                  var ruleName  =dateName+"_" +(userId?"_"+userId:"")+"_WorkLife";
                  var exp = 'cron('+min+' '+hour+' '+day+' '+month+' ? '+year+')';
                  var payloadworklife = {"rule":ruleName,
