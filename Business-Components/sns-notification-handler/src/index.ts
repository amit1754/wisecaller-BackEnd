...
const MESSAGE_TYPE_SUBSCRIPTION_CONFIRMATION: string =
  "SubscriptionConfirmation";
const MESSAGE_TYPE_NOTIFICATION: string = "Notification";

export const handleSNSNotificationHeader = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  if (topicARN && messageType) {
    ...
    if (req.headers[CONTENT_TYPE_HEADER]) {
      if (
        req.headers[CONTENT_TYPE_HEADER].toString().toLowerCase() !=
        "application/json"
      ) {
        console.log(
          "Content-Type request header: " + req.headers[CONTENT_TYPE_HEADER]
        );
    ...
    console.log(
      "Modified Content-Type request header " + req.headers[CONTENT_TYPE_HEADER]
    );
  }
