...
export const getUserByPhoneAndId = async (
  id: any,
  phone: string
): Promise<any> => {
  let user = await User.findOne({ _id: { $ne: id }, "phones.no": phone });
  return user;
};
...
export const findOneAndUpdate = async (
  ...
  let user = await User.findOneAndUpdate({ _id: id }, obj, options);
  return user;
};
...
export const findOneDeviceByTokenById = async (
  id: any,
  token: any
): Promise<any> => {
...
export const findOneDeviceAndUpdateById = async (
  id: any,
  payload: any,
  options: any
): Promise<any> => {
  await UserDevices.findOneAndUpdate({ _id: id }, { ...payload }, options);
};
